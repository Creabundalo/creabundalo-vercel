globalThis.M24Calibration = (() => {
  ...
  function sampleMatches(pair,{horizon=null,lens=null,direction=null,regime=null}={}){
  ...
  function calibrate({forecasts=[],outcomes=[],horizon=null,lens=null,direction=null,regime=null,minSamples=DEFAULT_MIN_SAMPLES}={}){
    const pairs=joinForecastOutcomes(forecasts,outcomes).filter(pair=>sampleMatches(pair,{horizon,lens,direction,regime}));
    ...
    return {
      ...
      filter:{horizon,lens,direction,regime},
      sampleSize:n,
      successes,
      minSamples,
      ...
      rule:enough?'Historical reference only; non-stationary markets can change.':'Do not display a probability before the minimum sample threshold is met.'
    };
  }

  function horizonTable({forecasts=[],outcomes=[],lens='m24',minSamples=DEFAULT_MIN_SAMPLES,horizons=['NOW','3D','2W','1M','2M']}={}){
    return horizons.map(horizon=>calibrate({forecasts,outcomes,horizon,lens,minSamples}));
  }
  ...
  return {DEFAULT_MIN_SAMPLES,joinForecastOutcomes,wilson95,calibrate,horizonTable,toRecordPayload};
})();
