...
const source=['m24-cases.js','m24-lab.js','m24-primary-lab.js','m24-cohort.js'].map(f=>fs.readFileSync(f,'utf8')).join('\n');
const test=`
(async()=>{
  ...
  const shapeCase=(bars,schema)=>{
    ...
    if(breakBar){breakBar.close=100;breakBar.low=95;}
    return bars;
  };
  ...
  const result=await M24Cohort.run({provider:fakeProvider,caseSchemas:M24Cases.list(),granularity:86400});
  check(result.total===4,'expected four registered cases');
  check(result.measured===4,'all test cases should be measured');
  ...
  check(JSON.stringify(assets)===JSON.stringify(['BTC','ETH','SOL']),'expected BTC/ETH/SOL case registry');
