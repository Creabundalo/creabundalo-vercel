...
(async()=>{
  ...
  const caseSchema=M24Cases.get('ETH-2021-2022-TOP-MARKDOWN');
  ...
  const summary={
    ...
    rule:'Source completeness is independent from whether the historical action candidate is directional. No live-order action exists.'
  };
  globalThis.__m24fs.writeFileSync('m24-eth-2021-source-snapshot.json',JSON.stringify(summary,null,2));
  check(result.meaning.asset==='ETH','meaning context asset mismatch');
  check(result.meaning.sources.every(x=>x.assets.includes('ETH')),'ETH meaning context leaked another asset');
  check(result.meaning.firstTop.count>=1&&result.meaning.secondTop.count>=1,'ETH top meaning coverage incomplete');
...
`;
vm.runInThisContext(`${source}\n${test}`,{filename:'m24-eth-source-e2e-bundle.js'});
